...
function ReadCustomUnitStats( pUnit:table, kSubjectData:table )	
	...
	-- if pUnit:GetProperty("LAST_CURRENT_XP") ~= kSubjectData.UnitExperience then pUnit:SetProperty("LAST_CURRENT_XP", kSubjectData.UnitExperience); end
	kSubjectData.ExperienceBalance = (pUnit:GetProperty("XP_BALANCE") ~= nil) and pUnit:GetProperty("XP_BALANCE") or 0;
	return kSubjectData;
end
...
function View( data )
	...
	if (not UILens.IsLensActive("Religion") and data.Combat > 0 and data.MaxExperience > 0) then
		local sTooltip = Locale.Lookup("LOC_HUD_UNIT_PANEL_XP_TT", (data.UnitExperience + data.ExperienceBalance), data.MaxExperience, data.UnitLevel + 1);
